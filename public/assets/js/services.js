...
function openModal(id = false) {
    ...
    let webForm = document.getElementById('serviceForm');
    webForm.querySelector('#title').value = '';
    webForm.querySelector('#category').value = '';
    ...
    webForm.querySelector('#iconPreviewImg').src = '';
        ...
        fetch(App.cust() + '/edit/' + `${id}`)
            ...
            .then(result => {
                if (result) {
                    ...
                    //  $('#category').val(result[0].type ?? '').trigger('change');
                    ...
                    if (result[0].icon) {
                        $('#selectedIconsPreview').removeClass('hidden');
                        webForm.querySelector('#iconPreviewImg').src = result[0].icon;
                    }
...
$(document).on('change', '#category', function (e, selectedSubcategoryId = null) {
    ...
    if (id) {
        $.ajax({
            url: App.getSiteurl() + 'admin/services/subcategories/' + id,
            type: "POST",
            dataType: 'json',
            success: function (res) {
                if (res.success) {
