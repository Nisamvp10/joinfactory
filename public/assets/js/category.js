...
function renderExpertiseTable() {
    ...
    if (allData.categories.length === 0) {
        html += `<div class="text-center py-8">
                    <h3 class="text-lg font-medium text-gray-700">No Data found</h3>
                    <p class="text-gray-500 mt-1">${(allData.message ? allData.message : 'Try adjusting your search')}</p>
                </div>`
    } else {
        ...
        paginatedData.forEach(function (item, idx) {
            html += `<tr data-id="${item.id}" class="${item.parent_id == 0 ? 'bg-green-100' : ''}">
                <td class="px-2 py-2 whitespace-nowrap border border-gray-300 serial">${start + idx + 1}</td> 
        ...
        let totalPages = Math.ceil(allData.categories.length / rowsPerPage);
        html += `
...
// Fix column width while dragging
...
function updateFullDataOrder() {
    ...
    $('#sortable tr').each(function () {
        ...
        let item = allData.categories.find(x => x.id == id);
        if (item) newPageItems.push(item);
    });

    // remove old items
    allData.categories.splice(start, rowsPerPage);

    // insert new order
    allData.categories.splice(start, 0, ...newPageItems);
